...
public final class SettingCreator {
    ...
    public static <I, O> Setting.Builder<I, O> createUserSetting(Class<I> input, Class<O> output, String settingKey, boolean required, String label, InputType inputType) {
    ...
    public static <I, O> void testFoo(TypeToken<I> input, TypeToken<O> output) {
        System.out.println(input.getType().getTypeName() +"\n"+output.getType().getTypeName());
    }

    public static <I, O> Setting.Builder<I, O> createRangeSliderSetting(Class<I> input, Class<O> output, String settingKey, boolean required, String label, InputType inputType, Number min, Number max, Number step) {
        RangeSliderInput<I, O> selectionInput = new RangeSliderInput<>(inputType, label, min, max, step);
        return new Setting.Builder<>(settingKey, required, selectionInput);
    }

    public static <I, O> Setting.Builder<I, O> createToggleSetting(Class<I> input, Class<O> output, String settingKey, boolean required, String label, InputType inputType) {
        ToggleInput<I, O> toggleInput = new ToggleInput<>(inputType, label);
        return new Setting.Builder<>(settingKey, required, toggleInput);
    }
