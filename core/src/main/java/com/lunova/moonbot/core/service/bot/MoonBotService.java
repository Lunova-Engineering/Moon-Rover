...
import com.lunova.moonbot.core.Configuration;
...
import com.lunova.moonbot.core.service.bot.event.EventDispatcher;
...
@ServiceInfo(name = "Moon Bot Service", critical = true, disabled = false, priority = 3)
@ExecutorConfig(
        corePoolSize = 0,
        maximumPoolSize = 1,
        keepAliveTime = 0,
...
public class MoonBotService extends Service<ServiceExecutor> {
     ...
     * with Discord, enables gateway intents, adds event listeners, and awaits the ready state of
     * the bot session.
     *
     * @throws ServiceLoadingException If there is an error during the initialization process, such
     *                                 as misconfiguration or interrupted execution.
     */
    @Override
    public void initialize() {
        try {
            botSession =
                    JDABuilder.createDefault(Configuration.getProperty("AUTH_TOKEN"))
            ...
            logger.error(
                    "Failed to start Moon-Bot service due to an {}.",
