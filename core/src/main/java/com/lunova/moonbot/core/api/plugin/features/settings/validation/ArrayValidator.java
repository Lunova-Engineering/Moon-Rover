...
public class ArrayValidator<T> extends Validator<ArrayList<T>> {
    ...
    protected ArrayValidator(String identifier, DataType dataType, List<ValidationRule<ArrayList<T>>> validationRules) {
    ...
    public static class Builder<T> {
        ...
        public Builder<T> setMaximumElements(int maximum) {
            rules.add(new MinimumValueRule<>(maximum) {
                @Override
                public boolean validateRule(List<T> target) {
                    return target.size() <= getValue().intValue();
                }
        ...
        public Builder<T> setElementRange(int minimum, int maximum) {
            rules.add(new RangeValueRule<>(minimum, maximum) {
                ...
                @Override
                public boolean validateRule(List<T> target) {
                    return target.size() >= getMin().intValue() && target.size() <= getMax().intValue();
                }
            });
            return this;
        }
