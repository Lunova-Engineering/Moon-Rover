...
public final class FeatureManager {
    ...
    @JsonIgnore private boolean isSealed = false;

    public void registerFeature(Feature feature) {
        FeatureContainer container =
                FeatureComponentFactory.createContainerWith(
                        feature.getName() + " Container", feature);
        registerFeatureContainer(container);
    }

    public void registerFeatureContainer(FeatureContainer container) {
        if (!isSealed) {
            ImmutableSet<Feature> unique =
                    container.getFeatures().stream()
                            .filter(feature -> !features.containsKey(feature.getClass()))
            ...
            throw new IllegalStateException(
                    "Cannot register containers after the FeatureManager has been sealed.");
        }
    }
